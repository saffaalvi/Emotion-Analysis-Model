...
def predict(path):
  ...
  output = "The model has predicted class/label " + np.array2string(prediction) + " which is the emotion " + labels[prediction]
  return output
...
def get_audio():
    ...
    transcript = speech_to_text('./output.wav')
    output = predict('./output.wav')
